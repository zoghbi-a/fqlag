...
def pfunc__lor0(f, p, log=True):
    """0-centered lorentzian function"""
    a, c = p[0], np.exp(p[1])
    if log:
        a = np.exp(a)
    return a * (c/(2*np.pi)) / ( f**2 + (c/2)**2 )

def pderv__lor0(f, p, log=True):
    """0-centered lorentzian derivative"""
    a, c = p[0], np.exp(p[1])
    fac = a
    if log:
        a = np.exp(a)
...
def identify_model(model, log=True):
    ...
    log: bool
        The model is based on the log of the norm or not
    ...
    if '+' in model:
        ...
        if not model in models:
            raise ValueError(f'model {model} is not part of built-in models: [{", ".join(models.keys())}]')
...
class Psdf(FqLagBin):
    ...
    fql: list
        [fq_min, fq_max] the minimum and maximum of the frequency
        range to be modeled. This should be wider than what the light cuve
        contains
    model: str
        string of a built-in model. See @identify_model function for 
        a list of built-in functions.
    log: bool
        Fit for the log of the normalization if True, otherwise fit for the linear.
    sigma: bool
        if True, also fit for additionaly variance (log-sigma), which is the last
        parameter in the parameter list
    dt: float
        sampling time of the light curves. If given, corrections to sampling
        bias is applied, otherwise, we don't apply it.
    NFQ: int
        The number of bins in the frequency grid used in the calculations.
    ...
    def __init__(self, tarr, yarr, yerr, fql, model='pl', log=True, sigma=False, dt=None, NFQ=8):
        ...
        self.psd_func = pfunc
        self.psd_derv = pderv
        self.params = {'fql':fql, 'model':model, 'log':log, 'dt':dt, 'NFQ':NFQ}
        self.npar   = npar
        self.sigma  = sigma
    ...
    def covariance(self, pars):
        ...
        ----------
        pars: np.ndarray
            parameters of the function that defines the psd/covariance.
        ...
        if self.sigma:
            cov[np.diag_indices(self.npoints)] += np.exp(pars[-1])
        return cov
    ...
    def covariance_derivative(self, pars):
        ...
        if self.sigma:
            dcov[np.diag_indices(self.npoints)] += np.exp(pars[-1])
            dcov = np.concatenate([dcov,np.expand_dims(
                np.diag(np.repeat(np.exp(pars[-1]), self.npoints)), 0)])
        return dcov
