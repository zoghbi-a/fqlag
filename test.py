...
import fqlag
from fqlag.misc import check_grad, optimize



def simulate_lc(n, dt, mu, lag, seed=3343):
    np.random.seed(seed)
    sim = az.SimLC(32)
    sim.add_model('broken_powerlaw', [1e-4, -1, -2, 5e-2])
...
def test_psd_sim():
    ...
    # so no using the factor of 2 is the correct way, and when doing the logs
...
def test_cxd():
    ...
    p0 = np.concatenate([np.min([p1,p2], 0)*0.7, p1*0+1])
    cmod = fqlag.Cxd([tarr, tarr[1:]], [rarr, sarr[1:]], [rerr, serr[1:]], fql, p1, p2, dt)
...
def test_lcxd():
    ...
    p0 = np.concatenate([np.min([p1,p2], 0)-3, p1*0+0.5])
    cmod = fqlag.lCxd([tarr, tarr[1:]], [rarr, sarr[1:]], [rerr, serr[1:]], fql, p1, p2, )

    #p0 = np.concatenate([p1, p2, np.min([p1,p2], 0)-3, p1*0+1])
...
def test_ppsi():
    ...
    p0 = np.concatenate([p1, p1*0+0.8, p1*0])
